    ...
    @else
        ...
        @if ($summary)
            ...
            <div class="portal-card" style="margin-bottom:1rem;">
                <div class="portal-label">{{ __('Profil Anak') }}</div>
            ...
            <div class="portal-grid portal-grid--2" style="margin-bottom:1rem;">
                ...
                <div class="portal-card">
                    <div class="portal-label">{{ __('Tagihan Belum Lunas') }}</div>
                    <div class="portal-metric">
                        Rp {{ number_format((float) $summary['pending_fees']['outstanding_amount'], 0, ',', '.') }}
                    </div>
                    <div class="portal-muted">
                        {{ __(':count tagihan menunggu pembayaran', ['count' => $summary['pending_fees']['count']]) }}
                    </div>
                </div>
                ...
                --}}
                <div class="portal-card">
                    <div class="portal-label">{{ __('Kehadiran Bulan Ini') }}</div>
                    ...
                    @else
                        <div style="font-size:1rem;font-weight:600;margin-top:.25rem;">
                            {{ __('Data kehadiran belum tersedia') }}
                        </div>
                        <div class="portal-muted">
                            {{ __('Pencatatan presensi belum tersedia pada tahap ini.') }}
                        </div>
                    @endif
                </div>
            </div>
