<div>
    ...
    @else
        <div class="portal-card" style="margin-bottom:1rem;">
            <div class="portal-label">{{ __('Hasil Ujian') }}</div>
            <div style="font-size:1.25rem;font-weight:700;overflow-wrap:anywhere;">
                {{ $result['title'] }}
            </div>
            <div class="portal-muted">
                {{ $result['subject_name'] ?? __('Mata pelajaran') }}
                @if ($result['class_name'])
                    — {{ __('Kelas') }} {{ $result['class_name'] }}
                @endif
            </div>
        </div>
        ...
        <div class="portal-card" style="margin-bottom:1rem;text-align:center;">
            ...
            <div class="portal-muted">{{ __('dari 100') }}</div>
        </div>
        ...
        <div class="portal-card">
            <ul class="portal-list">
                <li>
                    ...
                    <span class="portal-badge portal-badge--success">{{ __('Sudah dikumpulkan') }}</span>
                </li>
                <li>
                ...
                <li>
                    <span class="portal-muted">{{ __('Dimulai') }}</span>
                    <span>{{ $result['started_at']?->translatedFormat('d F Y H:i') ?? '—' }}</span>
                </li>
                <li>
                    <span class="portal-muted">{{ __('Durasi ujian') }}</span>
                    <span>{{ __(':count menit', ['count' => $result['duration_minutes']]) }}</span>
                </li>
            </ul>
            ...
            <p class="portal-muted" style="margin:1rem 0 0;">
                {{ __('Pembahasan per soal tidak ditampilkan di sini. Tanyakan kepada guru mata pelajaran Anda bila ingin membahas jawabannya.') }}
            </p>
